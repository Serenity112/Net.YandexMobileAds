...
1) Открываем YandexMobileAdsProxy.xcworkspace
Это уже готовый xcworkspace проект, где настроено почти всё что надо. Осталось скачать зависимости и запустить скрипт, но все флаги и настройки проставлены.
...
То есть на этом этапе нам надо скачать pod-файлы. Я их не стал включать в репозиторий, сами скачаются

2) Добавляем зависимости на pod-фреймворки.
В настройках xcodeproj проекте YandexMobileAdsProxy во вкладке General добавляем зависимости на .framework файлы, что мы скачали через pod. 
В разделе "Frameworks and Libraries" жмём +, и выбираем все .framework либы. Они там будут, если вы их скачали через pod install.
ОБЯЗАТЕЛЬНО помечаем все как "Do Not Embed"
...
4) Сборка проекта
Мы стремимся получить .xcframework для архитектур айфона + эмулятора, поэтому можно собирать любым удобным нам способом. Я делаю это через `xcodebuild archive`. В скрипте "generation.sh" всё есть, собираем два раза - для девайса и эмулятора.

5) Получаем итоговый .xcframework
...
Но вообще если вы не меняете код в swift классах, можно вообще не трогать код для ApiDefinitions.
...
4. Если что-то вылетает, ну сорри, делал по приколу.

5. МЕГА-ВНИМАНИЕ на try-catch в Main.cs у тестового проекта. Звучит смешно, но без этого ничего не сработает и будут краши в релизе. ОБЯЗАТЕЛЬНО ДЕЛАЕМ try-catch
